...
function MapelIcon({ icon }: { icon: string }) {
  ...
  if (n.includes("bio"))
    return (
      <svg
        ...
        strokeLinejoin="round"
      >
        <path d="M17 8C8 10 5.9 16.17 3.82 19.67L2 21l1-2C4.53 15.67 8 9 17 8z" />
        <path d="M6 21c0-3.5 2-6 6-9" />
      </svg>
    );
  if (n.includes("sej") || n.includes("his"))
    return (
      <svg
        width="26"
        height="26"
  ...
  if (n.includes("eko") || n.includes("aku"))
    return (
      <svg
        width="26"
        height="26"
...
export default function DashboardSiswa() {
  ...
  const [user, setUser] = useState<{ nama: string; kelas: string } | null>(
    null,
  );
  ...
  const handleJoinKelas = async () => {
    ...
    try {
      ...
      setJoinSuccess(
        `Berhasil bergabung ke kelas ${kelas.nama || kelas.mapel}!`,
      );
      setKodeInput("");
      reloadMapel();
    } catch (err: unknown) {
      setJoinError(
        err instanceof Error ? err.message : "Gagal bergabung. Coba lagi.",
      );
    } finally {
  ...
  const handleLeaveKelas = async () => {
    ...
    try {
      ...
      setLeaveError(
        err instanceof Error ? err.message : "Gagal keluar kelas. Coba lagi.",
      );
    } finally {
        ...
        <nav className={styles.nav}>
          {NAV_KEYS.map((key, i) => (
            <a
              key={key}
              href={NAV_HREFS[i]}
              className={`${styles.navItem} ${NAV_ACTIVE[i] ? styles.navActive : ""}`}
          ...
          <button
            ...
            title={!expanded ? t("nav.keluar") : undefined}
          >
            ...
            {expanded && (
              <span className={styles.navLabel}>{t("nav.keluar")}</span>
            )}
          </button>
        </nav>
      </aside>
      ...
      {showLeave && leaveTarget && (
        <div className={styles.modalOverlay} onClick={handleCloseLeave}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <p className={styles.modalDesc}>
              Yakin ingin keluar dari kelas <strong>{leaveTarget.mapel}</strong>
              ? Kamu perlu kode untuk bergabung lagi.
            </p>
            {leaveError && (
              <div
                style={{
        ...
        <div className={styles.modalOverlay} onClick={handleCloseJoin}>
          ...
          >
            <div className={styles.joinModalHeader}>
              ...
              <div>
                <h3 className={styles.joinModalTitle}>
                  {t("joinModal.title")}
                </h3>
                <p className={styles.joinModalSubtitle}>
            ...
            <div className={styles.joinModalBody}>
              ...
              <div className={styles.joinInputWrap}>
                ...
                <input
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && !joinLoading && handleJoinKelas()
                  }
                  maxLength={20}
                  autoFocus
          ...
          {mapelLoading ? (
            ...
            <div className={styles.grid}>
              {mapelList.map((mapel) => (
                <a
                  key={mapel.id}
                  href={`/dashboard/siswa/materi/${encodeURIComponent(mapel.mapel)}`}
              ...
              >
                <div className={styles.cardJoinInner}>
                  ...
                  <span className={styles.cardJoinLabel}>
                    {t("dashboard.gabungKelas")}
                  </span>
                  <span className={styles.cardJoinSub}>
                    {t("dashboard.gabungKelasSub")}
                  </span>
                </div>
